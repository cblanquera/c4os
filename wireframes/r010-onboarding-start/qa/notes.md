...
# R010 Onboarding And Start QA Notes

## Review Round 1 — 2026-07-17

- Confirmed every file path present in `wireframes/r009-settings/` is also present in `wireframes/r010-onboarding-start/`.
- Confirmed onboarding and start markup live in `index.html`, styles live in `styles.css`, and behavior plus top-level routing live in `script.js`.
- Confirmed the obsolete standalone `provider-setup.html` and `start.html` files are absent.
- Confirmed JavaScript syntax for `script.js` and generated `markdown.js`.
- Confirmed direct `#onboarding` and `#start` workflow routes, all three required workspace actions, and exactly three recent workspace rows.
- Confirmed no HTML or JavaScript link routes back to the r009 revision.
- Browser-visible verification remains pending after the architecture correction.

## Review Round 2 — 2026-07-17

- Confirmed route-scoped vertical overflow is enabled for both launch views while workspace and Settings overflow rules remain unchanged.
- Confirmed the start-screen Settings link and its unused CSS were removed.
- Confirmed the launch SPA now applies `text-decoration: none` to all contained links.
- Browser refresh at 1217 × 701 remains the final visual check for the annotated scrolling and link-decoration fixes.

## Review Round 3 — 2026-07-17

- Confirmed the global Terminal composer no longer branches into active-process output mutation.
- Confirmed every non-empty Terminal composer submission calls `appendUserMessage()` and `createTerminalArtifact()`.
- Confirmed process-specific input remains available only through the expanded Terminal session form.
- Confirmed the Terminal composer consistently presents a command prefix, command placeholder, and Run command action.
- Browser refresh and an `ls` submission remain the final visible interaction check.

## Review Round 4 — 2026-07-17

- Confirmed File artifacts now store shared `editing` and `draftContent` state.
- Confirmed inline and focused editor input events synchronize the shared draft.
- Confirmed focused File markup derives editability, controls, line numbers, and content from shared state.
- Confirmed Close synchronizes the focused draft before restoring and re-rendering the inline File response.
- Browser refresh and the annotated expand/edit/close sequence remain the final visible interaction check.

## Review Round 5 — 2026-07-17

- Confirmed Explorer child files now retain a shared edit draft in both focused and inline renderers.
- Confirmed `restoreChat()` copies the focused folder and child-file state into the owning Folder artifact before clearing focus.
- Confirmed `updateFolderCard()` then renders that retained state in the inline response artifact.
- Browser refresh and the Explorer → file → optional edit → X sequence remain the final visible interaction check.
