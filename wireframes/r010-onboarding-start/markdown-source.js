...
function rawUrlDecorations(view) {
  ...
  for (const visible of view.visibleRanges) {
    ...
    while (match) {
      ranges.push(Decoration.mark({ class: 'cm-markdown-raw-url' }).range(
...
const rawUrlPlugin = ViewPlugin.fromClass(class {
...
function createEditor(element, options) {
  ...
  const extensions = [
    ...
    EditorView.contentAttributes.of({
      'aria-label': settings.label || element.getAttribute('aria-label') || 'Message AI',
      'aria-multiline': 'true'
    }),
    ...
    Prec.highest(keymap.of([
      { key: 'Mod-b', run: function (view) { return toggleMarker(view, '**'); } },
      { key: 'Mod-i', run: function (view) { return toggleMarker(view, '*'); } },
      { key: 'Shift-Enter', run: insertComposerLine },
      { key: 'Enter', run: function (view) { return submitComposer(view, onSubmit); } }
    ].concat(historyKeymap, defaultKeymap)))
  ];
  const state = EditorState.create({ doc: settings.value || '', extensions: extensions });
