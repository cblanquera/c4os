# Response Artifact Modes Wireframe Specification

## Revision Summary

- Revision folder: `wireframes/r002-response-artifact-modes/`
- Revision: `r002-response-artifact-modes`
- Status: New major revision copied forward from `wireframes/r001-ai-chat-shell/specs.md`.
- Product area: Desktop AI chat composer modes, response artifacts, contextual replies, and right-pane artifact tabs.
- Requested scope:
  - Add persistent Chat, Files, Browser, and Terminal modes above the composer input.
  - Give each mode a purpose-specific input treatment.
  - Render Files, Browser, and Terminal output as Response Artifacts in the thread.
  - Add Copy and Reply actions to all user messages, assistant messages, and artifacts.
  - Replace the mode selector with a removable quoted-reference strip while replying.
  - Expand artifacts into persistent tabs in the right pane.
  - Update Browser and File artifacts in place when referenced by follow-up instructions.
  - Append new Terminal output artifacts while preserving one continuous terminal session.
- Explicitly deferred:
...
- Trigger: User concept discussion and annotated decisions dated 2026-07-15.
- Open questions affecting later revisions: None blocking this draft. Minor language and density decisions remain reviewable.

## Source Of Truth

- `wireframes/r001-ai-chat-shell/`
  - Contributes the grayscale three-panel desktop shell, resizable panels, central thread, fixed composer, model label, thinking-activity disclosure, prompt controls, and responsive behavior.
  - Changed in this revision: The right panel becomes an artifact workspace; the composer gains persistent modes; the thread gains artifacts and contextual actions.
...
- Components: Panel toggles, resize separators, messages, thinking activity, mode selector, mode-specific inputs, prompt controls, response artifact cards, message actions, quoted-reference strip, artifact tabs, artifact viewer, browser navigation, file viewer/editor states, terminal output, approval actions, notifier.
- Required states:
  - Chat, Files, Browser, and Terminal composer modes.
...
- Used by: `index.html`.
- Regions: Mode selector or quoted-reference strip, mode-specific input, shared lower toolbar, send/open/run action.
- Behavior: Selected mode persists after submissions. Reply temporarily replaces the mode selector and uses Chat-style natural-language input; submit or cancel restores the previous mode.
- Shared files: `styles.css`, `script.js`.
...
- Appears: Above the composer input when no reply target is active.
- Variants and states: Chat, Files, Browser, Terminal; selected, hover, and focus.
- Inputs and outputs: Activation changes the mode-specific input and primary action while persisting the selected mode in memory.
- Accessibility: Tablist/tab semantics, selected state, keyboard activation, labeled panels.
- Files: `styles.css`, `script.js`.

### Quoted Reply Strip

- Appears: Replaces the mode selector while replying.
...
- Shared anatomy: Full-width thinking-activity disclosure, assistant icon and model label row, constrained left-aligned bordered artifact surface with independently centered type icon, title and status/meta, header-level Expand control, compact preview, and a lower-left Copy/Reply action row beneath the card.
- Browser variant: Back, forward, refresh, address, read-only page preview, navigation-history indicator.
- File variant: Path, file type/version, read-only text preview, proposed-change state, approval actions, saved state.
- Terminal variant: Session/working directory, command, exit status, output; cards are immutable after creation.
...
- Trigger: Activate Chat, Files, Browser, or Terminal.
- Affected element: Mode selector, input surface, placeholder, primary action label/icon, and relevant supporting controls.
- Before: One persisted mode.
- After: Selected mode persists in memory.
- Visible result: Composer input changes immediately; thread remains unchanged.
...
- Affected element: Mode selector area and composer input.
- Before: Persisted mode visible.
- After: Mode selector is replaced with quoted context and the input becomes natural-language Chat; cancel restores prior mode.
...
- Visible result: URL, page title, preview content, and navigation count update.
...
- `styles.css`: Copied from `r001` then adapted. Retains grayscale tokens, base reset, shell, messages, composer, menus, and responsive rules. Adds mode selector, reply strip, artifact cards, hover actions, right-pane tabs/viewers, approval states, and notifier.
- `script.js`: Copied from `r001` then adapted. Retains panels, menus, attachments, microphone, and base conversation behavior. Adds in-memory mode state, reply target, artifact registry, terminal session, contextual copy, Browser/File in-place updates, immutable Terminal cards, right-pane tabs, and hash workflow initialization.
- `index.html`: Copied from `r001` then adapted as the single SPA screen with realistic initial artifacts and all interaction hooks.
- `workflows.html`: Copied from `r001` then adapted into four workflow entry points.
...
- Chat, Files, Browser, and Terminal modes visibly and functionally change the composer input.
...
- Reply replaces the mode selector with a removable quoted reference, uses natural-language input, and restores the previous mode after cancel or submit.
...
- This revision represents the complete first draft of the approved artifact-mode concept. Review feedback may refine density, labels, state transitions, or workflow emphasis before phase approval.
