...
function hub() {
  app.innerHTML = `
    <section class="hub">
      <h1>C4OS Functional Interface Draft</h1>
      <p>Round 2 desktop app draft. These pages simulate structure, state, and interaction intent only.</p>
  ...
  app.innerHTML = `
    <main class="start-screen" aria-labelledby="start-title">
      <section class="start-copy">
        <p class="eyebrow">No trusted project folder</p>
        <h1 id="start-title">Open a folder to start working</h1>
        <p>The first usable state requires a local project folder so file access, instructions, skills, runtime policy, approvals, and workspace persistence are scoped before prompting.</p>
        <div class="start-actions" aria-label="Project entry actions">
          <button class="primary">Open Folder</button>
          <button class="secondary">Clone Repository</button>
          <button class="secondary">Open Workspace File</button>
        </div>
      </section>
      <section class="recent-workspaces" aria-labelledby="recent-title">
        <h2 id="recent-title">Recent Folder-Backed Workspaces</h2>
        <a class="workspace-row" href="./new-session.html"><strong>c4os2</strong><span>Trusted</span></a>
        <a class="workspace-row" href="./new-session.html"><strong>agent-lab</strong><span>2 roots</span></a>
        <a class="workspace-row" href="./new-session.html"><strong>docs-workbench</strong><span>Saved</span></a>
      </section>
    </main>
  `;
}
...
function sidebar(activeSession = false) {
  return `
    <aside class="sidebar">
      <div class="search-shell"><span class="icon">${icon("search")}</span><input class="search" aria-label="Search" name="workspace-search" autocomplete="off" value="Search" readonly></div>
  ...
  return `
    <div class="composer">
      ...
      <div class="composer-controls">
        ...
        <button class="control-chip with-icon approve" aria-label="Approval policy: Approve for me">${icon("shield")}<span class="approve-label">Approve for me</span></button>
...
function message(id, text, type = "user") {
  ...
  if (length > 1000) {
    return `<article class="message ${type} message-clipped">
      ...
      <label for="message-${id}" data-open="Show Less" data-close="Show More" class="message-visible-trigger" tabindex="0"></label>
    </article>`;
  }
...
function providerRow(title, sub) {
  return `<div class="list-row"><div><div class="row-title">${title}</div><div class="row-sub">${sub}</div></div><button class="secondary">Edit</button><button class="toggle" aria-label="${title} enabled"></button></div>`;
}
...
function addProvider() {
  return `
    ...
    <form class="form">
      <div class="field"><label for="provider-type">Provider Type</label><select id="provider-type" name="provider-type" autocomplete="off"><option>OpenAI-compatible</option><option>OpenRouter</option><option>LiteLLM proxy</option></select></div>
      <div class="field"><label for="provider-label">Label</label><input id="provider-label" name="provider-label" autocomplete="off" value="OpenRouter - Personal"></div>
      <div class="field"><label for="api-base-url">API Base URL</label><input id="api-base-url" name="api-base-url" type="url" inputmode="url" autocomplete="off" value="https://openrouter.ai/api/v1"></div>
      <div class="field"><label for="api-key">API Key</label><input id="api-key" name="api-key" type="password" spellcheck="false" autocomplete="off" value="sk-****************"></div>
      <div class="field"><label for="auth-type">Auth</label><select id="auth-type" name="auth-type" autocomplete="off"><option>Bearer token</option><option>Custom header</option></select></div>
      <div class="field"><label for="provider-headers">Headers</label><textarea id="provider-headers" name="provider-headers" rows="5" spellcheck="false" autocomplete="off">{}</textarea></div>
...
function modelRow(title, provider) {
  return `<div class="list-row"><div><div class="row-title">${title}</div><div class="row-sub">${provider}</div></div><span class="pill">Enabled</span><button class="toggle" aria-label="${title} enabled"></button></div>`;
}
...
if (screen === "hub") hub();
else if (screen === "app-start") appStart();
