# Terminal Session Workspace Wireframe Specification

## Revision Summary

- Revision folder: `wireframes/r007-terminal-session-workspace/`
- Revision: `r007-terminal-session-workspace`
- Status: New major revision copied forward from `wireframes/r006-file-explorer-artifact/specs.md`.
...
- Explicitly deferred:
  ...
  - Multiple-file selection, real filesystem enumeration, browser sub-tabs, multiple reply targets, credential-specific states, detached native windows, and production security policy.
  - Left-panel content and navigation above the contextual Chat pane.
- Trigger: User request and annotated Terminal Expand/Stop decisions dated 2026-07-15.
...
- Shared anatomy: Full-width thinking-activity disclosure, artifact-type icon and model label row, constrained left-aligned bordered artifact surface with text-only identity where applicable, title and status/meta, compact preview, and a lower-left Copy/Reply action row beneath the card. Browser, File, and Terminal add header-level Expand.
...
- `styles.css`: Copied from `r006` and adapted with a full-height selected-process Terminal body, inline returned prompt, running indicators, Stop, and streaming card states.
- `script.js`: Copied from `r006` and adapted with one terminal-session state model shared by inline artifacts and the expanded viewer.
- `index.html`: Copied from `r006` and updated with a realistic running-process artifact carrying Stop and Expand.
