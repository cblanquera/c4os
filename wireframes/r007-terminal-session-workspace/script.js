(function () {
  ...
  function terminalHeaderActionsMarkup(entry) {
    return '<div class="artifact-header-actions">' +
  ...
  function terminalCardSurfaceMarkup(entry) {
    return '<div class="artifact-surface response-container"><header class="artifact__header"><div class="artifact__identity"><div><strong data-artifact-title>$ ' + escapeHtml(entry.command) + '</strong><span data-artifact-meta>' + escapeHtml(terminalSession.cwd) + ' · ' + terminalSession.id + '</span></div></div>' + terminalHeaderActionsMarkup(entry) + '</header>' +
      '<pre class="terminal-output" data-terminal-output>' + escapeHtml(entry.output) + '</pre><footer class="artifact__footer"><span data-terminal-result>' + terminalResultMarkup(entry) + '</span><span>' + terminalSession.id + '</span></footer></div>';
  }
  ...
  function terminalSessionMarkup(selectedId) {
    ...
    return '<section class="pane-view focus-artifact focus-artifact--terminal" aria-label="Expanded terminal session"><header class="terminal-session-header"><div><strong>Terminal</strong><span>' + escapeHtml(terminalSession.cwd) + ' · ' + terminalSession.id + '</span></div><div class="pane-view__actions"><button class="pane-action pane-action--icon terminal-stop" type="button" data-terminal-stop="' + escapeHtml(running ? entry.id : '') + '" aria-label="Stop current process" title="Stop process"' + (running ? '' : ' hidden') + '>' + icons.stop + '</button><button class="pane-action pane-action--icon" type="button" data-close-focused-artifact aria-label="Close Terminal artifact" title="Close">' + icons.close + '</button></div></header><div class="terminal-main-body" data-terminal-main-body data-terminal-entry="' + escapeHtml(entry ? entry.id : '') + '" data-status="' + escapeHtml(entry ? entry.status : 'completed') + '" role="log" aria-live="polite"><span class="terminal-main-status" data-terminal-session-result>' + (entry ? terminalResultMarkup(entry) : '') + '</span><pre class="terminal-main-output" data-terminal-session-output>' + escapeHtml(output) + '</pre>' + (running ? '' : terminalInlinePromptMarkup()) + '</div></section>';
  }
  ...
  function updateTerminalCard(entry) {
    ...
    if (card) {
      ...
      card.classList.toggle('artifact--running', entry.status === 'running');
      ...
      var duration = card.querySelector('.artifact-thinking summary span');
  ...
  function createTerminalArtifact(command) {
    ...
    var node = document.createElement('article');
    node.className = 'artifact artifact--terminal' + (running ? ' artifact--running' : '');
    node.setAttribute('data-artifact-card', '');
    node.setAttribute('data-artifact-id', id);
    ...
    node.tabIndex = 0;
    node.innerHTML = artifactPreambleMarkup(running ? '1 sec' : '6 sec', running ? 'The command is running in the active terminal session.' : 'I ran the requested command in the active terminal session.', running ? 'Streaming process output' : 'Captured the command output', 'terminal') + terminalCardSurfaceMarkup(entry) + actionsMarkup('terminal', id, 'Terminal · ' + command, entry.output);
    thread.appendChild(node);
    syncTerminalComposer();
  ...
  function focusArtifact(id) {
    var state = artifacts[id];
    if (!state || ['browser', 'file', 'folder', 'terminal'].indexOf(state.type) === -1) return;
