(function () {
  ...
  function renderFocusViewer() {
    ...
    if (state.type === 'browser') {
      ...
      focusViewer.innerHTML = '<section class="pane-view focus-artifact focus-artifact--file" aria-label="Focused file artifact"><header class="focus-view__header"><div><strong>' + escapeHtml(state.title) + '</strong><span>' + escapeHtml(state.path) + ' · Version ' + state.version + '</span></div><div class="pane-view__actions"><button class="pane-action pane-action--icon" type="button" data-focus-file-edit aria-label="Edit ' + escapeHtml(state.title) + '" title="Edit">' + icons.edit + '</button><button class="pane-action" type="button" data-focus-file-cancel hidden>Cancel</button><button class="pane-action pane-action--primary" type="button" data-focus-file-save hidden>Save</button><button class="pane-action pane-action--icon" type="button" data-close-focused-artifact aria-label="Close File artifact" title="Close">' + icons.close + '</button></div></header><textarea class="pane-file-editor" data-focus-file-editor readonly spellcheck="false">' + escapeHtml(state.content) + '</textarea></section>';
    }
  }
  ...
  function focusArtifact(id) {
    ...
    if (thread.parentElement !== leftChatPaneThread) leftChatPaneThread.appendChild(thread);
