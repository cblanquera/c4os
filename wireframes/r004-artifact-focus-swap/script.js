(function () {
  ...
  var workspaceStage = document.querySelector('[data-workspace-stage]');
  var threadHost = document.querySelector('[data-thread-host]');
  var focusViewer = document.querySelector('[data-focus-viewer]');
  var chatFrame = document.querySelector('[data-chat-frame]');
  var chatFrameThread = document.querySelector('[data-chat-frame-thread]');
  ...
  var frameGeometry = { width: 380, height: 420, left: null, top: null };
  ...
  function frameBounds() {
    var bounds = workspaceStage.getBoundingClientRect();
    return {
      width: bounds.width,
  ...
  function clampFrameGeometry() {
    if (!chatFrame || chatFrame.hidden || narrowQuery.matches) return;
    var bounds = frameBounds();
    var maxWidth = Math.max(320, bounds.width - bounds.margin * 2);
    var maxHeight = Math.max(280, bounds.height - bounds.bottomReserve - bounds.margin * 2);
    frameGeometry.width = Math.min(Math.max(320, frameGeometry.width), maxWidth);
    frameGeometry.height = Math.min(Math.max(280, frameGeometry.height), maxHeight);
    if (frameGeometry.left === null) frameGeometry.left = Math.max(bounds.margin, bounds.width - frameGeometry.width - 24);
    if (frameGeometry.top === null) frameGeometry.top = Math.max(bounds.margin, bounds.height - bounds.bottomReserve - frameGeometry.height - 24);
    frameGeometry.left = Math.min(Math.max(bounds.margin, frameGeometry.left), Math.max(bounds.margin, bounds.width - frameGeometry.width - bounds.margin));
    frameGeometry.top = Math.min(Math.max(bounds.margin, frameGeometry.top), Math.max(bounds.margin, bounds.height - bounds.bottomReserve - frameGeometry.height - bounds.margin));
    chatFrame.style.left = frameGeometry.left + 'px';
    chatFrame.style.top = frameGeometry.top + 'px';
    chatFrame.style.width = frameGeometry.width + 'px';
    chatFrame.style.height = frameGeometry.height + 'px';
  }
  ...
  function setFocusPlaceholder(id) {
    ...
    placeholder.textContent = (state.type === 'browser' ? 'Browser' : 'File') + ' open in the workspace · ' + state.title;
  ...
  chatFrame.addEventListener('pointerdown', function (event) {
    if (narrowQuery.matches) return;
    var isResize = event.target.closest('[data-chat-frame-resize]');
    var isDrag = event.target.closest('[data-chat-frame-drag]') && !event.target.closest('button');
    ...
    function moveFrame(moveEvent) {
      if (isResize) {
        frameGeometry.width = start.width + moveEvent.clientX - startX;
        frameGeometry.height = start.height + moveEvent.clientY - startY;
      } else {
        frameGeometry.left = start.left + moveEvent.clientX - startX;
        frameGeometry.top = start.top + moveEvent.clientY - startY;
      }
      clampFrameGeometry();
    }

    function stopFrameMove() {
      document.removeEventListener('pointermove', moveFrame);
      document.removeEventListener('pointerup', stopFrameMove);
      document.removeEventListener('pointercancel', stopFrameMove);
    }

    document.addEventListener('pointermove', moveFrame);
    document.addEventListener('pointerup', stopFrameMove);
    document.addEventListener('pointercancel', stopFrameMove);
  });
  ...
  shell.addEventListener('click', function (event) {
    ...
    if (event.target.closest('[data-focus-file-save]')) {
      ...
      activeFile.content = document.querySelector('[data-focus-file-editor]').value;
