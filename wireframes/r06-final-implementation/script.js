const defaultRoute = document.body.dataset.route || "app-start";
...
const routes = [
  ...
  ["Shell Foundation", "./shell-foundation.html", "Final shell baseline with one global header and no default right panel."],
  ["Same Side Replacement", "./same-side-replacement.html", "Same-side plugin icon clicks replace the visible panel."],
  ["Per Chat Restore", "./per-chat-restore.html", "Per-chat panel visibility restoration."],
  ["Resize Collision", "./resize-collision.html", "Resize behavior preserves the center pane before clamping panels."],
  ["Hidden Activity", "./hidden-activity.html", "Hidden compatible plugin activity indicator behavior."],
  ["Debug", "./debug.html", "Chat Debug command, tool call, result, and approval history."],
  ["Repair State", "./repair-state.html", "Invalid plugin shell layout repair state."],
  ["Settings", "./settings.html", "Settings center route and panel restore contract."],
  ["Coverage", "./coverage.html", "Batch 1 shell foundation route and state coverage."]
];
...
const icons = {
  ...
  bug: "M8 2l1.5 2h5L16 2M9 9h6m-8 4h10m-9 5h8M5 9H3m18 0h-2M5 15H3m18 0h-2M7 4h10v12a5 5 0 0 1-10 0Z",
  ...
  messages: "M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4Zm-5-8H8m8 4H8",
...
/* r05 final implementation feature renderer. Kept self-contained so r04 carry-forward routes can stay on the r06 renderer. */
const finalFeatureRenderer = (() => {
  ...
  document.addEventListener("click", (event) => {
    ...
    const routeButton = event.target.closest("a[data-route], button[data-route]");
  ...
  function renderFeatureRoute(routeName) {
    if (routeName && routeStates[routeName] && window.location.hash !== `#${routeName}`) {
      window.history.replaceState({ route: routeName }, "", `#${routeName}`);
    }
    render();
  }
  ...
  return {
    canRender(routeName) {
      return Boolean(routeStates[routeName]);
    },
    routeNames() {
      return Object.keys(routeStates);
    },
    render: renderFeatureRoute
  };
})();

function isKnownRoute(routeName) {
  return routeIds.has(routeName) || finalFeatureRenderer.canRender(routeName);
}
...
function renderRoute(nextRoute, options = {}) {
  ...
  if (route === "app-start") renderStart();
  else if (finalFeatureRenderer.canRender(route)) finalFeatureRenderer.render(route);
  else if (route === "coverage") renderCoverage();
  else if (route.startsWith("settings")) renderSettings(route === "settings" ? "settings-providers" : route);
  else renderFinalShell(route);
...
function bindSpaNavigation() {
  document.addEventListener("click", (event) => {
    ...
    const routeButton = event.target.closest("a[data-route], button[data-route]");
    const targetRoute = anchor?.dataset.routeTarget || routeButton?.dataset.route;
    if (!targetRoute || !isKnownRoute(targetRoute)) return;
    event.preventDefault();
...
const finalShellPlugins = {
  chats: { id: "chats", label: "Chats", icon: "messages", route: "per-chat-restore", side: "left" },
  files: { id: "files", label: "Files", icon: "folder", route: "same-side-replacement", side: "left" },
  browser: { id: "browser", label: "Browser", icon: "globe", route: "per-chat-restore", side: "right" },
  terminal: { id: "terminal", label: "Terminal", icon: "terminal", route: "same-side-replacement", side: "right" },
  debug: { id: "debug", label: "Debug", icon: "bug", route: "debug", side: "right" }
};

const batchOneStates = {
  "shell-foundation": { title: "Locate Tauri integration", subtitle: "c4os2", left: null, right: null },
  "same-side-replacement": { title: "Locate Tauri integration", subtitle: "c4os2", left: "files", right: "terminal" },
  "per-chat-restore": { title: "Draft wireframes", subtitle: "c4os2", left: "chats", right: "browser", activeSession: true },
  "resize-collision": { title: "Build shell state", subtitle: "c4os2", left: "files", right: "terminal", collision: true },
  "hidden-activity": { title: "Review hidden updates", subtitle: "c4os2", left: null, right: null, activity: "browser" },
  debug: { title: "Debug run history", subtitle: "c4os2", left: "chats", right: "debug", activeSession: true },
  "repair-state": { title: "Settings", subtitle: "Plugin repair", left: null, right: null, repair: true }
};

function shellState(screen) {
  if (batchOneStates[screen]) return { ...batchOneStates[screen], route: screen };
  const isSession = screen === "chat-session";
  const right = screen === "file-explorer" || screen === "file-editor"
    ? "files"
    : screen === "terminal"
      ? "terminal"
      : null;
  return {
    route: screen,
    title: isSession ? activeWorkspace.session : activeWorkspace.project,
    subtitle: screen === "file-explorer" || screen === "file-editor" ? "Files" : screen === "terminal" ? "Terminal" : "Workspace",
    left: "chats",
    right,
...
function renderFinalShell(screen) {
  const state = shellState(screen);
  const classes = [
    "final-shell",
    state.left ? "has-left" : "",
    state.right ? "has-right" : "",
    state.collision ? "is-collision-review" : ""
  ].filter(Boolean).join(" ");
  app.replaceChildren(
    h("div", { class: classes }, [
      finalHeader(state),
      state.left ? finalPluginPanel(state.left, "left", state) : null,
      state.left ? resizeHandle("left") : null,
      h("section", { class: "final-workbench", id: "main", tabindex: "-1" }, [finalCenter(state)]),
      state.right ? resizeHandle("right") : null,
      state.right ? finalPluginPanel(state.right, "right", state) : null
    ])
  );
  bindShowMore();
  bindComposerControls();
  bindTerminalResize();
  bindFinalShellResize();
}

function finalHeader(state) {
  return h("header", { class: "final-header" }, [
    h("nav", { class: "final-header-icons left", "aria-label": "Left plugin icons" }, [
      finalPluginButton(finalShellPlugins.chats, state),
      finalPluginButton(finalShellPlugins.files, state)
    ]),
    h("div", { class: "final-header-title" }, [
      h("strong", { text: state.title, translate: "no" }),
      h("span", { text: state.subtitle })
    ]),
    h("nav", { class: "final-header-icons right", "aria-label": "Right plugin icons" }, [
      finalPluginButton(finalShellPlugins.browser, state),
      finalPluginButton(finalShellPlugins.terminal, state),
      finalPluginButton(finalShellPlugins.debug, state),
      h("button", {
        class: `icon-button${route.startsWith("settings") ? " is-active" : ""}`,
        type: "button",
        "aria-label": "Settings",
        "aria-pressed": route.startsWith("settings") ? "true" : "false",
        "data-route": "settings"
      }, [svgIcon("settings")])
    ])
  ]);
...
function finalPluginButton(plugin, state) {
  const active = state.left === plugin.id || state.right === plugin.id;
  return h("button", {
    class: `icon-button${active ? " is-active" : ""}`,
    type: "button",
    "aria-label": plugin.label,
    "aria-pressed": active ? "true" : "false",
    "data-route": active ? "shell-foundation" : plugin.route
  }, [
    svgIcon(plugin.icon),
    state.activity === plugin.id ? h("span", { class: "activity-dot", "aria-hidden": "true" }) : null
  ]);
}

function finalPluginPanel(id, side, state) {
  const plugin = finalShellPlugins[id];
  return h("aside", { class: `final-plugin-panel ${side} ${id}`, "aria-label": `${plugin.label} panel` }, [
    h("div", { class: "final-panel-body" }, finalPanelBody(id, state))
  ]);
}

function finalPanelBody(id, state) {
  if (id === "chats") return finalChatsPanel(state);
  if (id === "files") return [filesTool(state.route === "file-editor")];
  if (id === "browser") return [browserTool()];
  if (id === "terminal") return [terminalTool()];
  return [debugTool()];
}

function finalChatsPanel(state) {
  const rows = [];
  projects.forEach((project) => {
    rows.push(projectRow(project, Boolean(state.activeSession)));
    (project.sessions || []).forEach((session) => rows.push(sessionRow(session, Boolean(state.activeSession) && session === activeWorkspace.session)));
  });
  return [
    h("label", { class: "final-search" }, [
      svgIcon("search"),
      h("span", { class: "sr-only", text: "Search chats" }),
      h("input", { type: "search", name: "chat-search", autocomplete: "off", placeholder: "Search chats" })
    ]),
...
function finalCenter(state) {
  if (state.repair) return repairWorkbench();
  if (state.activeSession || state.right === "debug") return renderThread();
  return renderEmptyWorkspace(state.legacyScreen || state.route);
}

function repairWorkbench() {
  return h("main", { class: "repair-workbench" }, [
    h("section", { class: "repair-card" }, [
      h("span", { class: "status-pill", text: "Plugin layout needs repair" }),
      h("h1", { text: "Review plugin shell settings" }),
      h("p", { text: "One plugin requested an unavailable side. C4OS disabled the panel contribution and kept the app shell usable." }),
      h("div", { class: "repair-actions" }, [
        h("button", { class: "button primary", type: "button" }, [svgIcon("settings"), h("span", { text: "Open plugin settings" })]),
        h("button", { class: "button secondary", type: "button" }, [h("span", { text: "Disable plugin" })])
      ])
    ])
  ]);
}
...
function debugTool() {
  return h("section", { class: "debug-tool" }, [
    h("nav", { class: "debug-tabs", "aria-label": "Chat Debug views" }, [
      h("a", { class: "debug-tab is-active", href: "#debug", text: "Current run" }),
      h("a", { class: "debug-tab", href: "#debug", text: "History" })
    ]),
    h("pre", { class: "debug-output", text: [
      "[09:42:11] run.started",
      "$ git log -1 --oneline",
      "a6d3c20 Add shell panel restore proof",
      "",
      "[09:42:14] tool_call_requested terminal.run",
      "status completed",
      "",
      "[09:42:18] approval_policy",
      "rule allow once"
    ].join("\n") }),
    h("div", { class: "debug-events" }, [
      debugEventRow("terminal.run", "Completed", "Command output recorded for this chat."),
      debugEventRow("tool_output_delta", "Stored", "Output is visible here without opening Terminal."),
      debugEventRow("approval_policy", "Applied", "Decision event is available for audit.")
    ])
  ]);
}

function debugEventRow(kind, status, detail) {
  return h("article", { class: "debug-event-row" }, [
    h("strong", { text: kind }),
    h("span", { text: status }),
    h("p", { text: detail })
  ]);
...
function bindFinalShellResize() {
  const shell = document.querySelector(".final-shell");
  if (!shell) return;
  shell.querySelectorAll("[data-resize-panel]").forEach((handle) => {
    handle.addEventListener("pointerdown", (event) => {
      const side = handle.dataset.resizePanel;
      const panel = side === "left" ? shell.querySelector(".final-plugin-panel.left") : shell.querySelector(".final-plugin-panel.right");
      if (!panel) return;
...
function renderCoverage() {
  app.replaceChildren(
    h("main", { id: "main", class: "coverage-main r06-coverage", tabindex: "-1" }, [
      h("h1", { text: "r06 Batch 1 coverage" }),
      h("p", { text: "This route isolates review coverage so shell routes stay product-like." }),
      h("table", { class: "coverage-table" }, [
        h("thead", {}, [h("tr", {}, ["Route", "Spec coverage", "Review intent"].map((text) => h("th", { text })))]),
        h("tbody", {}, [
          coverageRow("#shell-foundation", "01, 02", "Single global header, plugin icon groups, and no default right panel."),
          coverageRow("#same-side-replacement", "01, 02", "Same-side plugin icon clicks replace the current panel."),
          coverageRow("#per-chat-restore", "02", "Panel visibility restores per chat session."),
          coverageRow("#resize-collision", "02", "Resize state keeps the center pane usable before panel expansion."),
          coverageRow("#hidden-activity", "01, 02", "Compatible hidden plugins can show activity without stealing focus."),
          coverageRow("#repair-state", "01, 02", "Invalid shell layout degrades into repair/disable choices."),
          coverageRow("#debug", "01", "Chat Debug owns command, tool, result, and approval visibility.")
        ])
      ])
    ])
  );
...
function coverageRow(routeName, specs, intent) {
  return h("tr", {}, [
    h("td", {}, [h("a", { href: routeName, text: routeName })]),
    h("td", { text: specs }),
    h("td", { text: intent })
  ]);
}
