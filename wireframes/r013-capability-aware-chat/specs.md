# C4OS Capability-Aware Chat Wireframe Specification
...
- Requested scope:
  ...
  - Replace approval choices with `Ask for approval`, `Approve safe actions`, `Approve for me`, and `Custom` in Configuration and the Chat composer.
  ...
  - Offer explicit change-model, convert, remove, or cancel choices when a model switch conflicts with the current draft.
  - Recompute model-dependent session controls atomically and show reasoning effort only when the effective route supports it.
  - Present generic activity for ordinary work and a reasoning summary only when the effective route exposes one; never render a fabricated private chain of thought.
  - Show compact runtime, environment, workspace, and health provenance in the chat header and expandable provider/model/adapter/environment details per response.
  - Preserve r011 behavior while normalizing the existing HTML, CSS, and JavaScript.
  - Use stable component-oriented class names without changing `data-*` behavior hooks.
  - Alphabetize CSS properties, split multi-selector rules, and group styles with ownership comments.
  - Normalize HTML and JavaScript formatting and add comments where they clarify component ownership.
  ...
  - Center “What do you want to build in <project name>?” in that blank thread, then append and activate the sidebar chat only after its first valid submission.
  - Derive the initial chat-item title from the first prompt or first attachment filename and truncate it when needed.
  - Keep the workspace header to the active thread title only; remove the “Chat, files, browser, and terminal” subtitle from every thread state.
  - Give found project paths Reveal, Copy path, Rename, and Remove menu actions.
  - Give missing project paths Relocate, Copy path, Rename, and Remove menu actions.
  - Indicate a missing project path through lighter italic project text without a persistent Missing label.
  - Make project items sortable through drag and drop while retaining nested sessions.
  - Let the project list fill the full remaining height of the left panel rather than ending at its content height.
  - Use the supplied Projects peg for hierarchy and density while retaining the current grayscale wireframe language.
...
- Trigger: Frozen AI-harness research recommendation and user request to start the new revision, dated 2026-07-18.
- Open questions affecting later revisions: Whether the compact composer model picker needs price/latency signals in addition to capability filters; whether saved exceptions need a dedicated detail drawer after this structural round.

## Source Of Truth

- `.agents/context/runtime-session-architecture.md`
  - Accepted reusable truth for the four approval presets, model/session capability boundaries, chat binding, honest activity presentation, and compact provenance.
- `.agents/specs/00001-c4os-ai-harness-research/model-capabilities.md`
  - Defines capability layers, filters/details, attachment preflight, model-switch incompatibility handling, dynamic controls, and the no-fabricated-reasoning rule.
- `.agents/specs/00001-c4os-ai-harness-research/approval-policy-model.md`
  - Defines four presets, seven user-facing policy groups, concrete exceptions, and the `Approve for me` safety ceiling.
- `.agents/specs/00001-c4os-ai-harness-research/journeys.md`
  - Defines first-submit binding, normalized send preflight, approval presentation, and environment-qualified provenance.
- `wireframes/r012-cleanup/`
  - Supplies the complete visual and interaction baseline copied forward for this revision.
  - Preserved unchanged as the historical accepted artifact.
- User workspace-project request and supplied Projects peg, 2026-07-17.
  - Defines search-first ordering, Projects add action, nested session hierarchy, hover actions, found/missing menus, sorting, and folder-picker behavior.
  - The peg contributes hierarchy and density, not its dark visual styling.
- `wireframes/r010-onboarding-start/`
  - Contributes the complete accepted SPA, launch flow, Settings, artifact behavior, and visual system.
  - Copied forward as the implementation base for this revision.
...
- Layout: Full-window start surface with a compact header, three action cards, and a recent-workspace list. In the stacked narrow layout, each action icon top-aligns with its title and description block.
...
- Sequence: Open the work disclosure, show generic Starting/Working activity, expose a provider reasoning summary only when the effective route supplies one, collapse to the final elapsed label, and then reveal the responder and final output.
...
- Trigger: Activate Projects `+` or Relocate from a missing project menu.
- Affected elements: Hidden directory input and project list.
- Before: The list is unchanged or a project path is missing.
- After: Native directory selection appends a new found project or updates the targeted missing project.
- Visible result: The selected folder appears as a project with a starter chat session.
- Responsible module: Directory-picker controller in `script.js`.

### Manage A Project

- Trigger: Hover/focus a project, then activate New chat or More.
- Affected elements: Project row, blank thread state, nested sessions, and contextual popover.
- Before: Hover actions are hidden and the menu is closed.
- After: New chat opens an unsaved blank thread without changing the session list; More exposes path-dependent actions.
- Visible result: The center asks what the user wants to build in the selected project. Found projects show Reveal, Copy path, Rename, Remove; missing projects show Relocate, Copy path, Rename, Remove.
- Responsible module: Project action and popover controller in `script.js`.

### Promote A New Chat On First Submission

- Trigger: Submit the first Chat prompt, attachment, or combined draft while a project blank thread is active.
- Affected elements: Blank thread, conversation, owning project’s session list, active session state, and thread title.
- Before: The blank thread is unsaved and has no sidebar chat item.
- After: The first prompt or attachment filename becomes the chat title, a session item is inserted under the originating project, and the existing Chat submit path renders the conversation.
- Visible result: The blank state becomes a conversation and its new sidebar item is active. Titles longer than 48 characters are truncated with an ellipsis.
- Responsible module: Pending-thread promotion hook in the workspace-project controller plus the existing Chat submit controller in `script.js`.

### Sort Projects

- Trigger: Drag one project item before or after another.
- Affected elements: Project list order.
- Before: Original project sequence.
- After: Dragged project occupies the drop target position with its sessions and state intact.
- Responsible module: Native drag-and-drop project sorter in `script.js`.

### Remove A Chat Session

- Trigger: Hover/focus a chat session and activate Remove.
- Affected elements: Owning project’s session list.
- Before: Session exists.
- After: Session is removed; another session becomes active when necessary.
- Responsible module: Session action controller in `script.js`.
...
- Left-panel hooks: `data-session-search`, `data-project-add`, `data-project-list`, `data-project-*`, `data-session-*`, and `data-project-folder-input`.
...
- Historical Advanced Policies scope from 2026-07-17 defines the internal nine-group, 71-identity scenario corpus; r013 supersedes its user-facing presentation with seven groups and concrete exceptions.
...
- Customize category policies and exceptions: `./index.html#settings/advanced-policies`.
...
- Advanced Policies: seven-group rail, cross-group search, fixed trailing selects, concrete Exceptions view, dirty rows, Save Policies, and empty results.
...
- Models:
  - Search, provider filtering, and capability filtering define visible results.
  - Compact capability chips summarize Vision, Tools, Reasoning, Audio, and context size; Details exposes route-specific support and evidence.
  - Models default enabled; row switches update availability.
  - The bulk action reads Disable while visible results are enabled, or Enable when every visible result is disabled.
...
- Configuration:
  ...
  - Default Approval Policy offers the four accepted presets and explains the safety ceiling for Approve for me.
...
- Advanced Policies:
  ...
  - Search spans all seven user-facing groups.
  - Category rules and concrete Exceptions are separate views.
...
## R013 Authoritative Delta Contract

### Screen Inventory

- `workflows.html`
  - Purpose: Review launcher for capability-aware Chat, Models, Configuration, and Advanced Policies.
...
- `index.html#settings/configuration`
  - Purpose: Select one of four approval presets and understand the safety ceiling.
- `index.html#settings/advanced-policies`
  - Purpose: Configure seven broad category groups and review or revoke concrete exceptions.
  - States: category view, group selection, search results, dirty/save, exceptions view, and revoked exception.
...
- Compatibility alert: reason plus Use compatible model, Convert, Remove file, and Cancel.
- Model settings capability chips, capability filter, clickable model names, and details dialog.
- Approval preset selector and guardrail callout.
- Advanced Policy tabs, seven group controls, rule selectors, and exception rows.
...
- Model switching:
  ...
  - Conflict result: The model does not change; incompatible attachments remain visible; the alert names the model and files.
  - Compatible result: The model changes, all dependent controls are recomputed, attachment labels update, and stale reasoning values do not remain active.
- Attachment preflight:
  - Trigger: Add a file, change model, or send.
  - Result: Each attachment receives a route-specific status. Send is blocked only for unresolved incompatible content.
  - Resolution: Use compatible model changes to GPT-5; Convert marks only the affected prototype attachment as converted; Remove file deletes only affected items; Cancel preserves draft and current model.
- Reasoning:
  - GPT-5 exposes the effort control; GPT-5 fast and Kimi K2 hide it and force the prototype value to Off.
  - Assistant responses use Reasoning summary only for a supported route with effort enabled. Artifacts and unsupported routes use generic Activity.
- Provenance:
  - The Chat header exposes session provenance through a compact information control.
...
- Approval presets:
  - Configuration and the composer expose Ask for approval, Approve safe actions, Approve for me, and Custom.
  - Approve for me never removes the sandbox, trusted-root, maximum-authority, or managed-policy boundary.
- Advanced Policies:
  - Category rules expose Use default, Allow, Ask, and Deny.
  - Exceptions remain separate narrow records and may be revoked without changing category defaults.
...
- `workflows.html`: imports `./styles.css` and links to the four r013 review routes.
- `index.html`: imports `./styles.css`, `./markdown.js`, and `./script.js`; all r013 states remain hash-addressable within the static SPA.
...
- Advanced Policies renders exactly seven user-facing groups and a separate Exceptions view.
- Wide and narrow review widths have no document-level horizontal overflow after layout transitions settle.
- No browser console errors or warnings occur on the checked routes.
- Rendered product UI contains no review notes, TODOs, or implementation annotations.
