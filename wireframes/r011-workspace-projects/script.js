(function () {
  ...
  var artifacts = {
    ...
    'file-1': { id: 'file-1', type: 'file', title: 'notes.md', path: '~/project/docs/notes.md', content: sampleFileContent(), draftContent: null, version: 1, status: 'Viewing', proposedContent: '', editing: false },
  ...
  function renderDraftAttachments() {
    ...
    composer.dataset.firstAttachmentName = draftAttachments.length ? draftAttachments[0].name : '';
  ...
  function discardDraftAttachments() {
    var discardedUrls = draftAttachments.map(function (attachment) { return attachment.previewUrl; }).filter(Boolean);
    draftAttachments.forEach(function (attachment) {
      if (attachment.previewUrl) URL.revokeObjectURL(attachment.previewUrl);
    });
    draftAttachments = [];
    attachmentUrls = attachmentUrls.filter(function (url) { return discardedUrls.indexOf(url) === -1; });
    if (attachmentInput) attachmentInput.value = '';
    renderDraftAttachments();
  }
  ...
  function explorerFile(path) {
    ...
    return { title: title, path: path, content: sampleExplorerFileContent(title), draftContent: null, version: 1, editing: false };
  }
  ...
  function explorerFileBodyMarkup(file, focused) {
    var content = file.editing && file.draftContent != null ? file.draftContent : file.content;
    return '<div class="explorer-file-document' + (focused ? ' explorer-file-document--focused' : '') + '" data-explorer-file-document data-editing="' + String(file.editing) + '">' +
      '<pre class="explorer-file-document__lines" data-explorer-file-lines aria-hidden="true"' + (file.editing ? '' : ' hidden') + '>' + lineNumbers(content) + '</pre>' +
      '<div class="explorer-file-document__editor" data-explorer-file-editor role="textbox" aria-label="' + (file.editing ? 'Edit ' : 'View ') + escapeHtml(file.title) + '" aria-multiline="true" aria-readonly="' + String(!file.editing) + '" contenteditable="' + (file.editing ? 'true' : 'false') + '" spellcheck="false">' + escapeHtml(content) + '</div>' +
      '</div>';
  }
  ...
  function updateFileCard(id) {
    ...
    if (state.editing) setEditorText(card.querySelector('[data-file-editor-input]'), state.draftContent == null ? (state.proposedContent || state.content) : state.draftContent);
  ...
  function createFileArtifact(path) {
    ...
    artifacts[id] = { id: id, type: 'file', title: title, path: path.indexOf('/') > -1 ? path : '~/project/docs/' + path, content: sampleFileContent(), draftContent: null, version: 1, status: 'Viewing', proposedContent: '', editing: false };
    var frame = artifactFrameMarkup({
  ...
  function copyText(text) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(function () { showCopyNotice('Copied'); }, fallback);
    } else fallback();
  }
  ...
  function renderFocusViewer() {
    ...
    if (focusedExplorerFile) {
      ...
      var fileDraft = state.draftContent == null ? (state.proposedContent || state.content) : state.draftContent;
      focusViewer.innerHTML = focusArtifactMarkup(
        'file',
        ...
        '<header class="artifact-shell__header"><div class="artifact-shell__breadcrumbs">' +
          ...
          escapeHtml(state.title) + '" title="Edit"' + (state.editing ? ' hidden' : '') + '>' + icons.edit +
          ...
          escapeHtml(state.title) + '" title="Discard changes"' + (state.editing ? '' : ' hidden') + '>' + icons.trash +
          ...
          escapeHtml(state.title) + '" title="Save"' + (state.editing ? '' : ' hidden') + '>' + icons.save +
          ...
          icons.close + '</button></div></header><div class="focus-file-editor" data-focus-file-surface data-editing="' + String(state.editing) + '"><pre class="focus-file-editor__lines" data-focus-file-lines aria-hidden="true"' + (state.editing ? '' : ' hidden') + '>' +
          lineNumbers(fileDraft) +
          '</pre><div class="pane-file-editor" data-focus-file-editor role="textbox" aria-label="' + (state.editing ? 'Edit ' : 'View ') +
          escapeHtml(state.title) +
          '" aria-multiline="true" aria-readonly="' + String(!state.editing) + '" contenteditable="' + String(state.editing) + '" spellcheck="false">' +
          escapeHtml(fileDraft) + '</div></div>'
      );
    }
  }
  ...
  function restoreChat() {
    var closingFileId = focusedArtifact && artifacts[focusedArtifact] && artifacts[focusedArtifact].type === 'file' ? focusedArtifact : null;
    var closingFolderId = focusedArtifact && artifacts[focusedArtifact] && artifacts[focusedArtifact].type === 'folder' ? focusedArtifact : null;
    if (closingFileId && artifacts[closingFileId].editing) {
      artifacts[closingFileId].draftContent = editorText(focusViewer.querySelector('[data-focus-file-editor]'));
    }
    if (closingFolderId) {
      if (focusedExplorerFile && focusedExplorerFile.editing) {
        focusedExplorerFile.draftContent = editorText(focusViewer.querySelector('[data-explorer-file-editor]'));
      }
      artifacts[closingFolderId].path = focusedFolderPath || artifacts[closingFolderId].path;
  ...
  shell.addEventListener('click', function (event) {
    ...
    if (explorerEdit) {
      ...
      if (explorerFileState.draftContent == null) explorerFileState.draftContent = explorerFileState.content;
  ...
  shell.addEventListener('input', function (event) {
    if (event.target.matches('[data-file-editor-input]')) {
      ...
      if (inlineFileCard && artifacts[inlineFileCard.getAttribute('data-artifact-id')]) {
        artifacts[inlineFileCard.getAttribute('data-artifact-id')].draftContent = editorText(event.target);
      }
    ...
    if (event.target.matches('[data-focus-file-editor]')) {
      if (focusedArtifact && artifacts[focusedArtifact]) artifacts[focusedArtifact].draftContent = editorText(event.target);
    ...
    if (event.target.matches('[data-explorer-file-editor]')) {
      ...
      var activeExplorerFile = explorerCard
        ...
        : focusedExplorerFile;
      if (activeExplorerFile && activeExplorerFile.editing) activeExplorerFile.draftContent = editorText(event.target);
  ...
  shell.addEventListener('workspace:copy', function (event) {
    if (event.detail && typeof event.detail.text === 'string') copyText(event.detail.text);
  });
...
(() => {
  ...
  const notify = (message) => {
    ...
    notifyTimer = window.setTimeout(() => { notifier.hidden = true; }, 2400);
  };

  const activatePage = (slug, updateHash = true) => {
    const next = validPages.has(slug) ? slug : 'providers';
    pages.forEach((page) => { page.hidden = page.dataset.settingsPage !== next; });
    ...
    if (updateHash && window.location.hash !== `#settings/${next}`) history.replaceState(null, '', `#settings/${next}`);
  };

  navItems.forEach((item) => item.addEventListener('click', () => activatePage(item.dataset.settingsNav)));
  ...
  root.addEventListener('click', (event) => {
    ...
    if (providerRow) {
      ...
      syncProviderModels();
      notify(`${providerRow.querySelector('strong')?.textContent || 'Provider'} ${next ? 'enabled' : 'disabled'}`);
      return;
    }
  ...
  let savedRuntime = runtimeOptions.find((option) => option.getAttribute('aria-checked') === 'true')?.dataset.runtimeOption || 'opencode';
  ...
  runtimeOptions.forEach((option) => option.addEventListener('click', () => {
  ...
  runtimeSave?.addEventListener('click', () => {
    ...
    syncRuntimeSelection();
    const runtimeName = runtimeOptions.find((option) => option.dataset.runtimeOption === savedRuntime)?.querySelector('strong')?.textContent || 'Runtime';
  ...
  root.querySelectorAll('[data-dialog-open]').forEach((button) => button.addEventListener('click', () => openDialog(button.dataset.dialogOpen)));
  root.querySelectorAll('[data-dialog-close]').forEach((button) => button.addEventListener('click', closeDialog));
  dialogBackdrop?.addEventListener('click', (event) => { if (event.target === dialogBackdrop) closeDialog(); });
  document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && activeDialog) closeDialog(); });

  const providerDefaults = {
    openrouter: { type: 'OpenRouter', label: 'OpenRouter - Personal', url: 'https://openrouter.ai/api/v1', icon: 'OR' },
    huggingface: { type: 'Hugging Face', label: 'Hugging Face - Personal', url: 'https://router.huggingface.co/v1', icon: 'HF' },
    openai: { type: 'OpenAI', label: 'OpenAI - Personal', url: 'https://api.openai.com/v1', icon: 'AI' },
    compatible: { type: 'OpenAI Compatible', label: 'Local AI Server', url: 'http://127.0.0.1:4000/v1', icon: 'OA' },
  };
  ...
  const providerDialogTitle = root.querySelector('[data-provider-dialog-title]');
  const providerSubmit = root.querySelector('[data-provider-submit]');
  const providerBaseURL = providerForm?.querySelector('[data-provider-base-url]');
  const providerAuth = providerForm?.querySelector('[data-provider-auth]');
  const providerHeaderName = providerForm?.querySelector('[data-provider-header-name]');
  const providerAPIKey = providerForm?.querySelector('[data-provider-api-key]');
  const providerHeaders = providerForm?.querySelector('[data-provider-headers]');
  const providerKeyHelp = providerForm?.querySelector('[data-provider-key-help]');
  let editingProviderId = null;

  const updateProviderFields = ({ useDefaults = false } = {}) => {
    if (!providerForm) return;
    const type = providerDefaults[providerTypeSelect.value] ? providerTypeSelect.value : 'compatible';
    if (useDefaults) {
    ...
    providerForm.elements.apiKey.required = !editingProviderId && !(compatible && auth === 'none');
    if (providerKeyHelp) providerKeyHelp.textContent = editingProviderId ? 'Leave blank to keep the saved API key.' : 'Required for this provider.';
  ...
  const openProviderEdit = (row) => {
    ...
    const type = providerDefaults[row.dataset.providerType] ? row.dataset.providerType : 'compatible';
    providerTypeSelect.value = type;
    providerForm.elements.label.value = row.dataset.providerLabel || row.querySelector('strong')?.textContent || '';
    providerForm.elements.baseUrl.value = row.dataset.providerUrl || providerDefaults[type].url;
    providerForm.elements.apiKey.value = '';
    providerForm.elements.apiKey.type = 'password';
    providerForm.elements.auth.value = row.dataset.providerAuth || 'bearer';
    providerForm.elements.headerName.value = row.dataset.providerHeaderName || 'X-API-Key';
  ...
  root.querySelector('[data-provider-add]')?.addEventListener('click', openProviderAdd);
  providerTypeSelect?.addEventListener('change', () => updateProviderFields({ useDefaults: !editingProviderId }));
  ...
  const providerFormValid = () => {
    ...
    const required = [...providerForm.querySelectorAll('[required]')].filter((input) => !input.closest('label, [data-provider-field]')?.hidden);
    const invalid = required.find((input) => !input.value.trim());
    if (invalid) {
      invalid.focus();
      if (providerStatus) providerStatus.textContent = 'Enter all required connection details.';
      return false;
    }
    const label = providerForm.elements.label.value.trim().toLowerCase();
    const duplicate = [...root.querySelectorAll('[data-provider-row]')].find((row) => row.dataset.providerRow !== editingProviderId && (row.dataset.providerLabel || '').trim().toLowerCase() === label);
    if (duplicate) {
      providerForm.elements.label.focus();
      if (providerStatus) providerStatus.textContent = 'Provider labels must be unique.';
      return false;
    }
    if (providerTypeSelect.value === 'compatible') {
      try {
        ...
        if (providerStatus) providerStatus.textContent = 'Headers must be a valid JSON object.';
        return false;
      }
    }
    return true;
  ...
  root.querySelector('[data-provider-test]')?.addEventListener('click', () => {
    ...
    if (providerStatus) providerStatus.textContent = 'Connected successfully · Models are available.';
  });
  providerForm?.addEventListener('submit', (event) => {
    ...
    const type = providerDefaults[providerTypeSelect.value] ? providerTypeSelect.value : 'compatible';
    const defaults = providerDefaults[type];
    const url = type === 'compatible' ? providerForm.elements.baseUrl.value.trim() : defaults.url;
    const id = editingProviderId || label.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'custom';
    ...
    existing.dataset.providerAuth = type === 'compatible' ? providerForm.elements.auth.value : 'bearer';
    existing.dataset.providerHeaderName = type === 'compatible' ? providerForm.elements.headerName.value.trim() : '';
    existing.dataset.providerHeaders = type === 'compatible' ? providerForm.elements.headers.value.trim() : '{}';
  ...
  const modelProviderFilter = root.querySelector('[data-model-provider-filter]');
  ...
  function syncModelAvailability(row) {
    const enabled = row.querySelector('.switch')?.getAttribute('aria-checked') === 'true';
  ...
  function updateModelBulkAction() {
    ...
    const allDisabled = visible.length > 0 && visible.every((row) => row.querySelector('.switch')?.getAttribute('aria-checked') === 'false');
    modelBulkAction.disabled = visible.length === 0;
    modelBulkAction.textContent = allDisabled ? 'Enable results' : 'Disable results';
  }
  ...
  function filterModels() {
    ...
    modelRows.forEach((row) => {
      const provider = root.querySelector(`[data-provider-row="${row.dataset.modelProvider}"]`);
      const providerActive = provider?.dataset.providerActive === 'true';
      const providerMatch = providerValue === 'all' || row.dataset.modelProvider === providerValue;
      const searchMatch = !query || row.textContent.toLowerCase().includes(query);
      const match = providerActive && providerMatch && searchMatch;
      row.hidden = !match;
      if (match) visible += 1;
    });
  ...
  modelBulkAction?.addEventListener('click', () => {
    ...
    const enable = visible.every((row) => row.querySelector('.switch')?.getAttribute('aria-checked') === 'false');
    visible.forEach((row) => {
      row.querySelector('.switch')?.setAttribute('aria-checked', String(enable));
      syncModelAvailability(row);
    });
    updateModelBulkAction();
    notify(`${visible.length} visible model${visible.length === 1 ? '' : 's'} ${enable ? 'enabled' : 'disabled'}`);
  });
  root.querySelector('[data-refresh-models]')?.addEventListener('click', (event) => {
    const button = event.currentTarget;
    const previous = button.innerHTML;
    button.textContent = 'Refreshing…';
    button.disabled = true;
    window.setTimeout(() => { button.innerHTML = previous; button.disabled = false; notify('Models refreshed from active providers'); }, 700);
  });

  const setTabbedPanel = (kind, value) => {
    root.querySelectorAll(`[data-${kind}-tab]`).forEach((button) => button.setAttribute('aria-selected', String(button.dataset[`${kind}Tab`] === value)));
    root.querySelectorAll(`[data-${kind}-panel]`).forEach((panel) => { panel.hidden = panel.dataset[`${kind}Panel`] !== value; });
  };
  root.querySelectorAll('[data-plugin-tab]').forEach((button) => button.addEventListener('click', () => setTabbedPanel('plugin', button.dataset.pluginTab)));

  const pluginDetails = {
    github: { icon: 'GH', title: 'GitHub Workflow', description: 'Repository workflows for review, checks, and pull requests.', skills: 'PR Review, Fix CI, Address Comments', apps: 'GitHub', mcp: 'GitHub tools' },
    browser: { icon: 'WB', title: 'Web Research', description: 'Current-source research with a browser-backed workflow.', skills: 'Browser Research, Source Synthesis', apps: 'Browser', mcp: 'None' },
    analytics: { icon: 'AN', title: 'Analytics Workspace', description: 'Product analytics and spreadsheet reporting workflows.', skills: 'Analytics Audit, Funnel Review, Workbook Builder, QA', apps: 'Spreadsheets', mcp: 'None' },
    calendar: { icon: 'CA', title: 'Calendar Assistant', description: 'Meeting preparation, scheduling, and daily calendar briefs.', skills: 'Daily Brief, Meeting Prep, Group Scheduler, Free Up Time, Calendar', apps: 'Google Calendar', mcp: 'None' },
  };
  const installedPlugins = root.querySelector('[data-installed-plugins]');
  const installPluginFromDialog = root.querySelector('[data-plugin-install-dialog]');
  const uninstallPlugin = root.querySelector('[data-plugin-uninstall]');
  let activePluginId = null;
  const updateInstalledPluginCount = () => {
    const installedCount = root.querySelector('[data-plugin-tab="installed"] span');
    if (installedCount && installedPlugins) installedCount.textContent = String(installedPlugins.querySelectorAll('.extension-card').length);
  };
  const openPluginDetails = (id) => {
    ...
    root.querySelector('[data-plugin-dialog-body]').innerHTML = `<p>${data.description}</p><div class="dialog-capabilities"><div><strong>Skills</strong><span>${data.skills}</span></div><div><strong>Apps</strong><span>${data.apps}</span></div><div><strong>MCP</strong><span>${data.mcp}</span></div></div><nav class="plugin-legal-links" aria-label="${data.title} links"><a href="#" data-plugin-link="Website">Website</a><a href="#" data-plugin-link="Terms">Terms</a><a href="#" data-plugin-link="Privacy Policy">Privacy Policy</a></nav>`;
    const installed = !!installedPlugins?.querySelector(`[data-plugin-id="${id}"]`);
  ...
  root.querySelector('[data-plugin-dialog-body]')?.addEventListener('click', (event) => {
    const link = event.target.closest('[data-plugin-link]');
    if (!link) return;
    event.preventDefault();
    notify(`${link.dataset.pluginLink} opened for ${pluginDetails[activePluginId]?.title || 'plugin'}`);
  });
  root.querySelectorAll('[data-plugin-details]').forEach((button) => button.addEventListener('click', () => openPluginDetails(button.dataset.pluginDetails)));
  installPluginFromDialog?.addEventListener('click', () => {
    if (!activePluginId) return;
    const installButton = root.querySelector(`[data-plugin-install="${activePluginId}"]`);
  ...
  uninstallPlugin?.addEventListener('click', () => {
    ...
    installedPlugins.querySelector(`[data-plugin-id="${activePluginId}"]`)?.remove();
    const installButton = root.querySelector(`[data-plugin-install="${activePluginId}"]`);
    if (installButton) { installButton.textContent = 'Install'; installButton.disabled = false; }
    updateInstalledPluginCount();
    closeDialog();
    notify(`${name} uninstalled`);
  });
  root.querySelectorAll('[data-plugin-install]').forEach((button) => button.addEventListener('click', () => {
    const card = button.closest('.extension-card');
    if (card && installedPlugins && !installedPlugins.querySelector(`[data-plugin-id="${card.dataset.pluginId}"]`)) {
      ...
      toggle.setAttribute('aria-label', `Enable ${pluginDetails[button.dataset.pluginInstall]?.title || 'plugin'}`);
      toggle.innerHTML = '<span></span>';
      footer?.append(toggle);
      installedPlugins.append(installedCard);
      updateInstalledPluginCount();
      installedCard.querySelector('[data-plugin-details]')?.addEventListener('click', () => openPluginDetails(button.dataset.pluginInstall));
    }
    button.textContent = 'Installed';
    button.disabled = true;
    notify(`${pluginDetails[button.dataset.pluginInstall]?.title || 'Plugin'} installed`);
  }));
  root.querySelector('[data-plugin-search]')?.addEventListener('input', (event) => {
    const query = event.currentTarget.value.trim().toLowerCase();
    root.querySelectorAll('[data-plugin-directory] .extension-card').forEach((card) => { card.hidden = !!query && !card.textContent.toLowerCase().includes(query); });
  });
  ...
  root.querySelector('[data-marketplace-add]')?.addEventListener('click', () => {
  ...
  marketplaceForm?.addEventListener('submit', (event) => {
    ...
    if (!source) {
      marketplaceStatus.textContent = 'Enter a repository, Git URL, or local folder.';
  ...
  const skillData = [
    { id: 'frontend-design', name: 'Frontend Design', description: 'Build intentional, distinctive application interfaces from an approved structure.', overview: 'Use Frontend Design when an application surface needs a clear visual direction and a thoughtful interface system. It helps shape hierarchy, layout, component treatment, and interaction details while staying grounded in the approved product structure.', enabled: true },
    { id: 'workspace-review', name: 'Workspace Review', description: 'Review workspace changes, validation evidence, and unresolved decisions.', overview: 'Workspace Review provides a focused pass over local changes and their supporting evidence. It identifies mismatches, missing validation, and unresolved decisions before work is presented or handed off.', enabled: true },
    { id: 'document-editor', name: 'Document Editor', description: 'Create and revise structured documents with layout-aware checks.', overview: 'Document Editor supports drafting and revision where document structure and presentation both matter. It keeps headings, tables, pagination, and review-ready layout in view throughout the editing workflow.', enabled: false },
    { id: 'browser-automation', name: 'Browser Automation', description: 'Exercise real browser workflows and capture review evidence.', overview: 'Browser Automation runs realistic browser interactions for review and verification. It can navigate flows, exercise controls, inspect visible states, and capture evidence from the rendered experience.', enabled: true },
    { id: 'analytics', name: 'Analytics', description: 'Set up, improve, or audit product analytics and measurement.', overview: 'Analytics helps define meaningful events, properties, funnels, and quality checks. Use it when measurement needs to be planned, reviewed, or aligned with product decisions.', enabled: true },
    { id: 'ab-testing', name: 'A/B Testing', description: 'Plan experiments, variants, guardrails, and rollout decisions.', overview: 'Use A/B Testing when a product, page, onboarding step, pricing flow, or campaign needs a clear experiment design. It helps define hypotheses, variants, success metrics, guardrails, and rollout decisions.', enabled: true },
  ];
  ...
  const skillDialogDescription = root.querySelector('[data-skill-dialog-description]');
  const skillDialogOverview = root.querySelector('[data-skill-dialog-overview]');
  const skillDialogToggle = root.querySelector('[data-skill-dialog-toggle]');
  const skillIcon = '<span class="skill-circle-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"></path><path d="M14 2v6h6"></path></svg></span>';
  const renderSkills = () => {
    if (!skillList) return;
    const query = skillSearch?.value.trim().toLowerCase() || '';
    const list = skillData.filter((skill) => !query || `${skill.name} ${skill.description}`.toLowerCase().includes(query));
    skillList.hidden = list.length === 0;
    if (skillEmpty) skillEmpty.hidden = list.length !== 0;
    skillList.innerHTML = list.map((skill) => `<article class="skill-row" data-skill-id="${skill.id}"><button class="skill-row-main" type="button" data-skill-open aria-label="View ${skill.name} details">${skillIcon}<span class="skill-row-copy"><strong>${skill.name}</strong><span>${skill.description}</span></span></button><button class="switch" type="button" role="switch" aria-checked="${skill.enabled}" aria-label="Make ${skill.name} available in this project" data-skill-toggle><span></span></button></article>`).join('');
  };
  const openSkillDialog = (skill) => {
    ...
    skillDialogToggle.setAttribute('aria-label', `Make ${skill.name} available in this project`);
  ...
  skillList?.addEventListener('click', (event) => {
    ...
    if (event.target.closest('[data-skill-toggle]')) {
      ...
      renderSkills();
      notify(`${skill.name} ${skill.enabled ? 'available' : 'unavailable'} in this project`);
      return;
    }
  ...
  root.querySelector('[data-skill-uninstall]')?.addEventListener('click', () => {
  ...
  const mcpServers = [
    { id: 'node-repl', name: 'node_repl', transport: 'stdio', command: 'node-repl-mcp', arguments: ['--workspace', '.'], environment: [], passthrough: ['PATH'], workingDirectory: '~/code', enabled: true },
    { id: 'openai-developer-docs', name: 'openaiDeveloperDocs', transport: 'http', url: 'https://developers.openai.com/mcp', bearerTokenEnv: 'OPENAI_DOCS_TOKEN', headers: [], environmentHeaders: [], enabled: true },
    { id: 'stackpress-blog-mcp', name: 'stackpress_blog_mcp', transport: 'stdio', command: 'stackpress-blog-mcp', arguments: ['serve'], environment: [{ key: 'NODE_ENV', value: 'development' }], passthrough: ['PATH'], workingDirectory: '~/code/stackpress-blog', enabled: true },
  ];
  ...
  const mcpSettingsIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3"></circle><path d="M12 2v3M12 19v3M4.93 4.93l2.12 2.12M16.95 16.95l2.12 2.12M2 12h3M19 12h3M4.93 19.07l2.12-2.12M16.95 7.05l2.12-2.12"></path></svg>';
  const mcpRemoveIcon = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6M10 11v5M14 11v5"></path></svg>';
  const renderMcpServers = () => {
    if (!mcpList) return;
    mcpList.hidden = mcpServers.length === 0;
    if (mcpEmpty) mcpEmpty.hidden = mcpServers.length !== 0;
    mcpList.innerHTML = mcpServers.map((server) => `<article class="mcp-row" data-mcp-row="${server.id}"><strong>${escapeHTML(server.name)}</strong><button class="icon-button mcp-configure" type="button" data-mcp-configure aria-label="Configure ${escapeHTML(server.name)}">${mcpSettingsIcon}</button><button class="switch" type="button" role="switch" aria-checked="${server.enabled}" aria-label="Enable ${escapeHTML(server.name)}" data-mcp-toggle><span></span></button></article>`).join('');
  };
  const setMcpTransport = (transport) => {
    const next = transport === 'http' ? 'http' : 'stdio';
    if (mcpTransport) mcpTransport.value = next;
    root.querySelectorAll('[data-mcp-transport-option]').forEach((button) => button.setAttribute('aria-pressed', String(button.dataset.mcpTransportOption === next)));
    root.querySelectorAll('[data-mcp-panel]').forEach((panel) => { panel.hidden = panel.dataset.mcpPanel !== next; });
  ...
  const appendMcpRepeaterRow = (type, value = {}) => {
    ...
    const pair = type === 'environment' || type === 'header' || type === 'environmentHeader';
    const placeholders = type === 'environmentHeader' ? ['Header', 'Environment variable'] : ['Key', 'Value'];
    const fields = pair
    ...
    repeater.insertAdjacentHTML('beforeend', `<div class="mcp-repeat-row${pair ? ' mcp-repeat-row--pair' : ''}" data-mcp-repeat-row>${fields}<button class="mcp-remove-row" type="button" data-mcp-remove-row aria-label="Remove ${type}">${mcpRemoveIcon}</button></div>`);
  };
  const resetMcpRepeaters = (server = {}) => {
    ...
    Object.entries(values).forEach(([type, rows]) => {
      ...
      (rows.length ? rows : [type === 'environment' || type === 'header' || type === 'environmentHeader' ? { key: '', value: '' } : '']).forEach((row) => appendMcpRepeaterRow(type, row));
    });
  };
  const openMcpDialog = (server = null) => {
    if (!mcpForm) return;
    editingMcpId = server?.id || null;
    mcpDialogTitle.textContent = server ? 'Edit MCP server' : 'Connect to a custom MCP';
  ...
  const collectMcpRows = (type) => [...root.querySelectorAll(`[data-mcp-repeater="${type}"] [data-mcp-repeat-row]`)].map((row) => {
  ...
  }).filter((value) => typeof value === 'string' ? value : value.key || value.value);
  root.querySelector('[data-mcp-add-server]')?.addEventListener('click', () => openMcpDialog());
  root.querySelector('[data-mcp-learn]')?.addEventListener('click', (event) => { event.preventDefault(); notify('Opening MCP documentation'); });
  root.querySelectorAll('[data-mcp-transport-option]').forEach((button) => button.addEventListener('click', () => setMcpTransport(button.dataset.mcpTransportOption)));
  root.querySelectorAll('[data-mcp-add-row]').forEach((button) => button.addEventListener('click', () => appendMcpRepeaterRow(button.dataset.mcpAddRow)));
  ...
  mcpForm?.addEventListener('submit', (event) => {
    ...
    const requiredValue = transport === 'stdio' ? mcpForm.elements.command.value.trim() : mcpForm.elements.url.value.trim();
    const duplicate = mcpServers.find((server) => server.id !== editingMcpId && server.name.toLowerCase() === name.toLowerCase());
    if (!name || !requiredValue) {
      if (mcpStatus) mcpStatus.textContent = `Enter a name and ${transport === 'stdio' ? 'command' : 'URL'}.`;
      return;
    }
    if (duplicate) {
    ...
    const server = editingMcpId ? mcpServers.find((item) => item.id === editingMcpId) : { id: `mcp-${Date.now()}`, enabled: true };
  ...
  root.querySelector('[data-config-open-external]')?.addEventListener('click', () => notify('Opening config.toml externally'));
...
(() => {
  ...
  const groups = [
    { id: 'terminal', label: 'Terminal', items: [
      ['terminal.project.read', 'Runs a non-mutating shell command whose working directory and readable targets stay within the trusted project.'],
      ['terminal.project.write', 'Runs a command that can create, modify, move, or delete content inside the trusted project.'],
      ['terminal.external.read', 'Runs a command that reads files, metadata, processes, or system state outside the trusted project.'],
      ['terminal.external.write', 'Runs a command that can change files, applications, processes, configuration, or system state outside the trusted project.'],
      ['terminal.network.read', 'Runs a command that retrieves remote information without intentionally changing the remote resource.'],
      ['terminal.network.write', 'Runs a command that sends, publishes, uploads, deploys, or otherwise mutates a remote resource.'],
      ['terminal.unknown', 'The command cannot be classified confidently, combines ambiguous shell constructs, or has dynamically resolved targets.'],
    ]},
    { id: 'git', label: 'Git', items: [
      ['git.project.read', 'Reads repository state inside a trusted project, such as status, log, diff, show, branches, or blame.'],
      ['git.external.read', 'Reads a local repository outside the trusted project.'],
      ['git.remote.read', 'Reads from a remote Git service without changing remote state.'],
      ['git.project.write', 'Changes repository or worktree state inside the trusted project.'],
      ['git.external.write', 'Changes a local repository or worktree outside the trusted project.'],
      ['git.remote.write', 'Changes remote Git state, including pushes, remote deletions, tags, or repository mutation.'],
      ['git.unknown', 'The Git operation, repository location, or remote effect cannot be determined safely.'],
    ]},
    { id: 'filesystem', label: 'Filesystem', items: [
      ['file.project.read', 'Reads or lists files, folders, metadata, links, or search results inside the trusted project.'],
      ['file.external.read', 'Reads or lists filesystem content outside the trusted project.'],
      ['file.project.write', 'Creates or modifies files and folders inside the trusted project.'],
      ['file.external.write', 'Creates or modifies files and folders outside the trusted project.'],
      ['file.project.delete', 'Deletes or trashes content inside the trusted project.'],
      ['file.external.delete', 'Deletes or trashes content outside the trusted project.'],
      ['file.project.watch', 'Watches trusted-project paths for filesystem changes.'],
      ['file.external.watch', 'Watches paths outside the trusted project.'],
      ['file.unknown', 'The resolved path, scope, link destination, or resulting filesystem effect is unknown.'],
    ]},
    { id: 'browser', label: 'Browser', items: [
      ['browser.load', 'Loads a public URL or permitted local document using a non-mutating navigation request.'],
      ['browser.post', 'Sends form data or another request capable of changing remote state.'],
      ['browser.navigation', 'Changes browser location or history through navigation, redirects, or link activation.'],
      ['browser.use', 'Interacts with rendered page content, including clicking, typing, selecting, scrolling, or submitting forms.'],
      ['browser.read', 'Reads page text, DOM state, accessibility state, metadata, links, or visible content.'],
      ['browser.capture', 'Captures a screenshot, selected region, page representation, or annotation attachment.'],
      ['browser.download', 'Downloads remote content into the local filesystem.'],
      ['browser.upload', 'Sends a local file or generated artifact to a website.'],
      ['browser.local.read', 'Opens or reads a local file through the Browser surface.'],
      ['browser.auth.use', 'Uses an existing authenticated browser profile or logged-in session.'],
      ['browser.unknown', 'The navigation, interaction, target, authentication impact, or remote mutation cannot be classified safely.'],
    ]},
    { id: 'network', label: 'Network', items: [
      ['network.read', 'Retrieves remote data without intending to mutate the remote system.'],
      ['network.write', 'Creates, updates, submits, publishes, or deletes remote data.'],
      ['network.listen', 'Opens a local listening port or server.'],
      ['network.connect', 'Establishes a socket or service connection not adequately described as an ordinary read.'],
      ['network.unknown', 'The destination or remote effect cannot be determined.'],
    ]},
    { id: 'credentials', label: 'Credentials', items: [
      ['credential.use', 'Uses a stored secret through a governed operation without exposing its raw value.'],
      ['credential.create', 'Adds or imports a new credential into secure storage.'],
      ['credential.update', 'Replaces or changes a stored credential.'],
      ['credential.delete', 'Removes a credential from secure storage.'],
      ['credential.reveal', 'Exposes raw secret material to a caller or user.'],
      ['credential.unknown', 'The secret, consumer, destination, or manner of use cannot be determined.'],
    ]},
    { id: 'processes', label: 'Processes & apps', items: [
      ['process.read', 'Inspects running processes or application state.'],
      ['process.start', 'Starts a process, service, executable, or desktop application.'],
      ['process.control', 'Sends input, signals, focus changes, or other control operations to a running process.'],
      ['process.stop', 'Terminates a process or service.'],
      ['process.unknown', 'The executable, ownership, or resulting effect cannot be determined.'],
    ]},
    { id: 'desktop', label: 'Desktop facilities', items: [
      ...
      ['system.read', 'Reads OS, hardware, display, environment, or application information.'],
      ['system.write', 'Changes system or application-level configuration.'],
      ['app.open', 'Opens a file, URL, or resource in another desktop application.'],
      ['app.control', 'Interacts with or controls another desktop application.'],
    ]},
    { id: 'c4os', label: 'C4OS authority', items: [
      ['config.read', 'Reads non-sensitive C4OS configuration.'],
      ['config.write', 'Changes C4OS configuration or policy.'],
      ['plugin.read', 'Reads installed plugin metadata, status, or declared capabilities.'],
      ['plugin.write', 'Installs, removes, enables, disables, repairs, or reconfigures a plugin.'],
      ['extension.use', 'Activates extension-provided runtime behavior.'],
      ['mcp.read', 'Lists or reads MCP tools, resources, prompts, or server metadata.'],
      ['mcp.use', 'Invokes a non-mutating MCP operation.'],
      ['mcp.write', 'Invokes an MCP operation that can mutate external state.'],
      ['artifact.read', 'Reads or previews an existing C4OS artifact.'],
      ['artifact.write', 'Creates or modifies an artifact record.'],
      ['share.export', 'Exports, publishes, shares, or sends local content outside C4OS.'],
  ...
  const allItems = groups.flatMap((group) => group.items.map(([key, description]) => ({ key, description, groupId: group.id, groupLabel: group.label })));
  let activeGroup = groups[0].id;
  let savedPolicies = Object.fromEntries(allItems.map((item) => [item.key, 'default']));
  ...
  const isDirty = () => allItems.some((item) => draftPolicies[item.key] !== savedPolicies[item.key]);
  const notify = (message) => {
    if (!notifier) return;
    notifier.textContent = message;
    notifier.hidden = false;
  ...
  const renderGroups = () => {
    groupRoot.innerHTML = groups.map((group) => `<button class="policy-group" type="button" data-policy-group="${group.id}" aria-current="${group.id === activeGroup}"><span>${escapeHTML(group.label)}</span><span>${group.items.length}</span></button>`).join('');
  };
  ...
  const renderPolicies = () => {
    ...
    const items = query
      ? allItems.filter((item) => `${item.key} ${item.description} ${item.groupLabel}`.toLowerCase().includes(query))
      : group.items.map(([key, description]) => ({ key, description, groupId: group.id, groupLabel: group.label }));

    title.textContent = query ? 'Search results' : group.label;
    summary.textContent = query ? 'Matching identities across all policy groups.' : 'Each override is evaluated against the matching tool call.';
    ...
    listRoot.innerHTML = items.map((item) => {
      ...
      const optionMarkup = options.map(([value, label]) => `<option value="${value}"${draftPolicies[item.key] === value ? ' selected' : ''}>${label}</option>`).join('');
...
(function () {
  ...
  var defaults = {
    openrouter: { label: 'OpenRouter - Personal', url: 'https://openrouter.ai/api/v1' },
    huggingface: { label: 'Hugging Face - Personal', url: 'https://router.huggingface.co/v1' },
  ...
  function isVisible(element) { return element && !element.hidden; }
  ...
  type.addEventListener('change', function () { syncFields(true); });
  auth.addEventListener('change', function () { syncFields(false); });
  form.querySelector('[data-onboarding-password-toggle]').addEventListener('click', function (event) {
  ...
  form.addEventListener('submit', function (event) {
    event.preventDefault();
    if (!validateForm()) return;
    try { window.localStorage.setItem('c4os-provider-configured', 'true'); } catch (error) { /* Static preview may restrict storage. */ }
  ...
  launch.querySelectorAll('[data-workspace-action]').forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      window.setTimeout(function () { window.location.hash = '#chat'; }, 380);
    });
  });
...
(function () {
  ...
  if (activeSession && thread) threadSnapshots[activeSession.dataset.sessionId] = thread.innerHTML;
  ...
  function syncProjectLabels(project) {
    ...
    menuTrigger.setAttribute('aria-label', 'More actions for ' + (missing ? 'missing ' + name + ' project' : name));
  ...
  function saveActiveThread() {
    if (activeSession && thread && !pendingProject) threadSnapshots[activeSession.dataset.sessionId] = thread.innerHTML;
  }
  ...
  function restoreSavedThread(session) {
    ...
    else if (Object.keys(threadSnapshots).length) thread.innerHTML = threadSnapshots[Object.keys(threadSnapshots)[0]];
  ...
  function setActiveSession(session, restoreThread) {
    ...
    list.querySelectorAll('.session-item--active').forEach(function (item) { item.classList.remove('session-item--active'); });
  ...
  function showNoActiveSession() {
    ...
    if (emptyThreadHeading) emptyThreadHeading.textContent = 'Choose a project to start a chat';
  ...
  function beginNewThread(project) {
    ...
    if (thread) thread.hidden = true;
    if (emptyThread) emptyThread.hidden = false;
    if (emptyThreadHeading) emptyThreadHeading.textContent = 'What do you want to build in ' + projectName(project) + '?';
    var title = document.querySelector('.thread-title strong');
  ...
  function sessionMarkup(id, title) {
    ...
    return '<div class="session-item" data-session-id="' + id + '" data-session-title="' + title + '"><button type="button" data-session-open>' + title + '</button><button type="button" data-session-remove aria-label="Remove ' + title + '" title="Remove"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m18 6-12 12M6 6l12 12"></path></svg></button></div>';
  }
  ...
  function projectMarkup(id, name, path) {
    ...
    return '<article class="project-item" data-project-id="' + id + '" data-project-path="' + path + '" data-project-found="true" data-project-expanded="true" draggable="true">' +
      '<div class="project-row"><button class="project-toggle" type="button" data-project-toggle aria-expanded="true"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7h6l2 2h10v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2Z"></path></svg><span>' + name + '</span></button>' +
      '<div class="project-actions"><button type="button" data-project-menu-trigger aria-haspopup="menu" aria-expanded="false" aria-label="More actions for ' + name + '" title="More"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="5" cy="12" r="1"></circle><circle cx="12" cy="12" r="1"></circle><circle cx="19" cy="12" r="1"></circle></svg></button><button type="button" data-project-new-chat aria-label="New chat in ' + name + '" title="New chat"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 20h9"></path><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L8 18l-4 1 1-4Z"></path></svg></button></div>' +
      '<div class="project-menu" data-project-menu role="menu" hidden><button type="button" role="menuitem" data-project-action="reveal">Reveal</button><button type="button" role="menuitem" data-project-action="copy">Copy path</button><button type="button" role="menuitem" data-project-action="rename">Rename</button><span></span><button class="project-menu__danger" type="button" role="menuitem" data-project-action="remove">Remove</button></div></div>' +
      '<div class="project-sessions" data-project-sessions>' + sessionMarkup('session-' + id + '-welcome', 'New chat') + '</div></article>';
  }
  ...
  function addProjectFromSelection(files) {
    ...
    if (relocateProject && relocateProject.isConnected) {
      ...
      var missing = relocateProject.querySelector('[data-project-toggle] small');
    ...
    var id = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'project';
  ...
  function renameProject(project) {
    var toggle = project.querySelector('[data-project-toggle]');
    var row = project.querySelector('.project-row');
    ...
    toggle.hidden = true;
    row.insertBefore(input, row.querySelector('.project-actions'));
    input.focus();
    input.select();
    ...
    function finish(save) {
      ...
      toggle.hidden = false;
      input.remove();
      notify(save && next ? 'Renamed project to “' + next + '”' : 'Rename cancelled');
    }
  ...
  function copyPath(project) {
    ...
    if (shell) shell.dispatchEvent(new CustomEvent('workspace:copy', { detail: { text: path } }));
  }

  /** Removes a project and reconciles any active or pending workspace state. */
  function removeProject(project) {
    var removedName = projectName(project);
    var removedActive = Boolean(activeSession && project.contains(activeSession));
    ...
    if (removedPending || (removedActive && !pendingProject)) activateFallbackSession();
    notify('Removed “' + removedName + '”');
  }

  /** Removes one session and selects a valid fallback when it was active. */
  function removeSession(session) {
    var wasActive = activeSession === session || session.classList.contains('session-item--active');
  ...
  function filterSessions() {
    ...
    list.querySelectorAll('[data-project-id]').forEach(function (project) {
      var matches = 0;
      project.querySelectorAll('[data-session-id]').forEach(function (session) {
        var match = !query || (session.dataset.sessionTitle || '').toLowerCase().includes(query);
        session.hidden = !match;
        if (match) matches += 1;
      });
      project.hidden = Boolean(query && !matches);
      var sessions = project.querySelector('[data-project-sessions]');
      sessions.hidden = query ? !matches : project.dataset.projectExpanded !== 'true';
    });
  }
  ...
  folderInput.addEventListener('change', function () { addProjectFromSelection(folderInput.files); });
  search.addEventListener('input', filterSessions);
  if (composer) composer.addEventListener('submit', function () {
    if (!pendingProject || composer.dataset.mode !== 'chat') return;
    var input = document.querySelector('[data-mode-input="chat"]');
    var value = input && window.MarkdownRuntime ? window.MarkdownRuntime.fromElement(input).trim() : '';
    var hasAttachments = Number(composer.dataset.attachmentCount || 0) > 0;
    if (!value && !hasAttachments) return;

    var project = pendingProject;
    var title = promptTitle(value || composer.dataset.firstAttachmentName || 'New chat');
    var sessions = project.querySelector('[data-project-sessions]');
  ...
  list.addEventListener('click', function (event) {
    ...
    if (action) {
      ...
      closeMenus();
      if (kind === 'reveal') notify('Revealed “' + projectName(project) + '” in Finder');
      if (kind === 'copy') copyPath(project);
      if (kind === 'rename') renameProject(project);
      if (kind === 'remove') removeProject(project);
      if (kind === 'relocate') { relocateProject = project; folderInput.value = ''; folderInput.click(); }
      return;
    }
    if (newChat) {
    ...
    if (openSession) { setActiveSession(openSession.closest('[data-session-id]')); return; }
    if (toggle && !toggle.querySelector('input')) setProjectExpanded(project, project.dataset.projectExpanded !== 'true');
  });
  ...
  list.addEventListener('dragover', function (event) {
    ...
    list.querySelectorAll('[data-drag-over]').forEach(function (item) { item.removeAttribute('data-drag-over'); });
  ...
  document.addEventListener('click', function (event) {
    ...
    else if (!event.target.closest('[data-project-menu]') && !event.target.closest('[data-project-menu-trigger]')) closeMenus();
  });
...
(function () {
  ...
  var launch = document.querySelector('[data-launch-spa]');
  var launchViews = launch ? Array.prototype.slice.call(launch.querySelectorAll('[data-launch-view]')) : [];
  if (!workspace || !settings || !launch) return;

  function hasConfiguredProvider() {
    try { return window.localStorage.getItem('c4os-provider-configured') === 'true'; }
    catch (error) { return false; }
  }

  function syncAppRoute() {
    if (!window.location.hash) window.location.hash = hasConfiguredProvider() ? '#start' : '#onboarding';
    ...
    launchViews.forEach(function (view) { view.hidden = view.dataset.launchView !== route; });
    document.body.dataset.appView = settingsActive ? 'settings' : launchActive ? route : 'workspace';
  }
