...
const themeCss = readFileSync(
  resolve("src/features/platform/platform-theme.css"),
  "utf8",
);
const runtimeCss = readFileSync(
  resolve("src/features/runtime/runtime-qa.css"),
  "utf8",
);
const productCss = readFileSync(resolve("src/styles.css"), "utf8");
...
describe("semantic style contract", () => {
  ...
  it("keeps literal colors inside the one semantic token source", () => {
    for (const [label, css] of [
      ["src/styles.css", productCss],
      ["src/features/runtime/runtime-qa.css", runtimeCss],
    ] as const) {
      expect(
