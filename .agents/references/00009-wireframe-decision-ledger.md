...
- R5 added the accepted provider header above composer model filters, an in-place configured-provider chooser, provider-scoped model return, active-model preservation while browsing, and Escape/focus restoration.
- R6 accepted the provider label inline with its back chevron, removal of provider-profile subtitles from the compact chooser, and the divider plus `+ Create New` beneath the branch choices.
- R7 accepted replacing Projects with flat session-title results while search is non-empty, identifying each owning Project, preserving the query when a result opens, and restoring the unchanged hierarchy through an explicit clear control or Escape. `#chat-search` is the deterministic review route; the center workspace remains the selected Chat.
- The user accepted the complete r013 revision through R7 on 2026-07-18. r013 supersedes r012 as the forward behavioral and visual-interaction baseline; r012 remains the historical parity profile. Provider data, evidence timestamps, conversions, token usage, service calls, and persistence in the wireframe remain simulated rather than implementation evidence.
