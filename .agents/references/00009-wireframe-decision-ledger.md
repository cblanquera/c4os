...
- R5 added the user-requested provider header above composer model filters, an in-place configured-provider chooser, provider-scoped model return, active-model preservation while browsing, and Escape/focus restoration. The interaction decision is accepted; its rendered refinement remains in review until Round 5 approval.
- R6 kept the provider label inline with its back chevron, removed provider-profile subtitles from the compact chooser, and added the requested divider plus `+ Create New` beneath the branch choices. These annotated refinements remain in review until Round 6 approval.
- R7 replaces Projects with flat session-title results while search is non-empty, preserves the query when a result opens, and restores the unchanged hierarchy through an explicit clear control or Escape. `#chat-search` is the deterministic review route; the rendered case study remains in review until Round 7 approval.
