...
- Non-Batch-1 wireframes, remaining proofs, and executable acceptance are still pending where named by this spec. Do not treat this spec as implementation-ready until those records are created, reviewed, and accepted or explicitly deferred.
...
- Approved r05 Batch 1 shell-foundation wireframe evidence is recorded in
  `evidence.md` and covers the shell/plugin view-host behavior needed for
  pending final-implementation spec review. This closes only the approved
  Batch 1 wireframe gap; it does not freeze this spec or start implementation.
