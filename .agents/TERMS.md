...
- **Artifact Context Snapshot**: the immutable, bounded, type-specific state captured when a user replies to a Response Artifact. It is attached to the User Turn with a stable artifact reference; later reads and effects remain C4OS-brokered and freshness-checked.
...
- **C4OS Home**: the user-level `~/.c4os` application configuration and local-state root. It contains the last loaded Workspace as an unpacked working copy, app-level MCP/skill/plugin configuration, marketplace configuration, C4OS `config.toml`, and the protected Browser Environment profile registry. C4OS-managed credentials remain vault-only. On macOS, public WebKit owns raw website-data bytes in its platform-managed app container; C4OS Home owns their scope-to-profile identifiers and lifecycle. Website-controlled browser state is excluded from ordinary configuration, Workspace archives, normal exports, diagnostics, and model context.
...
- **Chat Session**: a Workspace- and Project-bound conversation whose configuration, cache, and archive records are stored in the owning Workspace. A Chat Session owns its Terminal shell and Response Artifacts unless a narrower artifact rule applies.
...
- **Inactive Record**: a retained Chat, Project, or Workspace record excluded from active product surfaces by the Remove action. Inactivation is not deletion, purge, export, or process termination.
...
- **Project**: a folder and trusted execution-root reference inside a Workspace. The same folder may have different Project configuration and Chat Sessions in different Workspaces; those overlays are Workspace-owned rather than written into the folder by default.
...
- **Workspace**: a portable C4OS zip archive, similar in role to a VS Code workspace, that stores an ordered list of Projects, Workspace-level configuration, per-Project configuration, and per-Chat configuration, cache, and archive records. Loading it reconstructs the main-screen state from an unpacked working copy under C4OS Home.
