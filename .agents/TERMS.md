...
- **C4OS app plugin**: a Codex-compatible plugin bundle with optional C4OS app-shell contributions. It is distinct from a Tauri plugin.
- **Persistent shell**: the C4OS-owned chat, session, settings, navigation, approval, attachment, and plugin-mounting frame.
- **Tool gateway**: the C4OS-owned authority that registers tools, applies policy, executes approved calls, records lifecycle events, and stores shared tool-result state.
- **Project folder**: a trusted local folder registered with C4OS and used as the durable identity for project-owned chats.
- **Workspace file**: an explicit load/save grouping of project-folder references; it is not the owner of project chats.
