...
13. Freeze the MVP in `.agents/archive/planning-corpus/mvp/mvp-freeze.md`.
...
Root-level files under `.agents/archive/planning-corpus/` are for lifecycle-wide planning documents that do not belong to one specialized folder. For example, a pre-plan brief belongs at `.agents/archive/planning-corpus/preplan-brief.md` because it frames the entire planning effort before specifications are generated.
...
`.agents/archive/planning-corpus/mvp/mvp-freeze.md` is now the implementation contract. Features not listed there are out of scope for the first implementation pass.
...
The final grill session belongs here as `.agents/archive/planning-corpus/reviews/final-implementation-readiness-review.md`. It should challenge the plan against the existing documents and classify findings strongly enough that blocker and high-risk items can be resolved before implementation planning.
...
Implementation epics, task breakdowns, dependencies, build order, and sprint plans. This folder should be created only after the final readiness review has been resolved and `.agents/archive/planning-corpus/mvp/mvp-freeze.md` exists.
...
- Do not create `implementation/` epics or tasks before `.agents/archive/planning-corpus/mvp/mvp-freeze.md` exists.
...
1. Read `CONTEXT.md`, this file, `.agents/archive/planning-corpus/mvp/mvp-freeze.md`, the current sprint or task plan, and the relevant acceptance criteria.
...
- `.agents/archive/planning-corpus/preplan-brief.md` exists and is historical context. Later MVP and freeze documents supersede it where they conflict.
- `.agents/archive/planning-corpus/mvp/mvp-freeze.md` and `.agents/archive/planning-corpus/implementation/` now exist because final readiness findings were resolved through the validation loop.
- The original lifecycle mentions "Open Questions and Research Gaps" under `.agents/archive/planning-corpus/reviews`, while the current tree has `.agents/archive/planning-corpus/research/open-questions-and-research-gaps.md`. That is reasonable if the file is treated as research inventory, but keep the convention consistent going forward.
