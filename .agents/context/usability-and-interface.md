...
- Accepted native feasibility currently covers macOS 26.5.1 arm64. macOS uses a native initial-theme snapshot when available and an independent live webview `prefers-color-scheme` listener because Tauri's native theme-change event did not fire in the accepted Proof. Windows and Linux native feasibility is deferred to a separate spec and must not inherit the macOS result.
