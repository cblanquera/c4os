...
Independently reconcile implementation, verification, Agent Acceptance, coverage, evidence, source-of-truth boundaries, context-promotion decisions, validator health, worktree scope, and coherent local history before terminal completion.

## Implementation Steps

1. Trace each of the 50 Feature Coverage IDs from Frozen requirement to primary/supporting task, production code, focused test, integrated rendered evidence, and passed Agent Acceptance.
2. Confirm all 15 tasks and three side quests are `verified`, never `accepted`, and contain the exact acceptance override plus concrete passed evidence.
3. Audit research/blocker closure, proof and wireframe authority boundaries, prohibited/deferred features, unresolved limitations, and external gates.
4. Perform the Context promotion/defer/no-change review and update accepted Context only for genuinely reusable accepted truth using the required workflow.
5. Run the Agent Workspace validator, documentation/link checks, full regression set, diff checks, worktree inspection, and local commit-history audit.
6. Reopen any task or coverage ID whose evidence is missing, simulated, stale, or contradicted.

## Verification Process

- Machine-readable counts and link checks for 50 IDs, 18 task files, statuses, exact acceptance lines, results, and evidence references.
- Independent source/test/rendered-evidence sampling across every coverage family.
- Final validator, git diff/status/log, build identity, and evidence-index inspection.
