...
- **Residual risk:** A crash after credential creation but before durable profile publication can leave an encrypted orphan credential. It contains no plaintext and grants no route authority, but Task 00015A must classify it. Integrated update/diagnostic recovery and the final native/accessibility audit remain owned by Tasks 00014 through 00015C.
...
- **Sources:** Exact local lockfiles and production source/tests; `.agents/references/00007-launch-and-settings-contract.md`; `output/native/task-00013-acceptance.md`. No new online source was required because the decision composes already frozen local authority and version contracts.
