...
| ID | Requirement |
| --- | --- |
| REQ-001 | Make approval popover/dialog functional for deny, deny-and-wait, allow-once, and allow-and-remember decisions, including Advanced metadata, remembered-rule summaries that show tool, action/risk category, target scope where applicable, plugin id for plugin tools, and session-only versus user-global duration. |
| REQ-002 | Support $ skills, @ plugin resources/files, and / runtime/gateway commands through trigger-scoped typeahead, caret-scoped active queries, keyboard selection, resolved inline references, and unresolved token fallback. |
...
| REQ-007 | Route authoritative prompt tag resolution, serialized prompt generation, and `/` command execution through backend/gateway services rather than frontend-only parsing. |
