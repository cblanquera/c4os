...
- [Load the detailed shell-plugin authority diagram, glossary, lifecycle states, and migration failure table](../../references/00005-shell-plugin-architecture-model.md) when reviewing component ownership, plugin state transitions, recovery behavior, or exact architecture terminology.

### DEC-001: 001: C4OS Grill Question 001 - Plugin Packaging Boundary

Source: `.agents/resources/grill/final-implementation/answers/001-c4os-grill-question-001-plugin-packaging-boundary.json`
...
Source: `.agents/resources/grill/final-implementation/answers/002-c4os-grill-question-002-plugin-backend-authority.json`
...
Source: `.agents/resources/grill/final-implementation/answers/002a-c4os-grill-question-002a-tauri-tool-authority-boundary.json`
...
Source: `.agents/resources/grill/final-implementation/answers/002b-c4os-grill-question-002b-runtime-tool-discovery-and-invocation.json`
...
Source: `.agents/resources/grill/final-implementation/answers/017-c4os-grill-question-017-c4os-plugin-manifest.json`
...
Source: `.agents/resources/grill/final-implementation/answers/038-c4os-grill-question-038-codex-plugin-compatibility-boundary.json`
...
Source: `.agents/resources/grill/final-implementation/answers/039-c4os-grill-question-039-plugin-marketplace-and-lifecycle.json`
...
Source: `.agents/resources/grill/final-implementation/answers/040-c4os-grill-question-040-plugin-backend-registration-boundary.json`
...
Source: `.agents/resources/grill/final-implementation/answers/050-c4os-grill-question-050-plugin-migration-failure-handling.json`
...
### DEC-013: Backend App Architect Resolution - Shell Plugin Architecture

Source: 2026-07-02 user architect profile in active chat.

  - Standards precedence: When shell/plugin architecture behavior is uncertain,
    resolve toward OpenAI/Codex conventions first, Claude/Anthropic conventions
    second, and broader open standards such as MCP third. C4OS-specific
    deviations must be explicit in context or spec records.
  - MCP-shaped tools: Tauri/native tools follow an MCP-shaped structural
    pattern: stable tool IDs, descriptions, typed input/output contracts,
    capability metadata, approval policy, structured results, lifecycle events,
    and inspectable state. C4OS remains the host/gateway and does not hand
    execution authority to plugin views.
  - Event-driven architecture: Shell, runtime, gateway, registry, lifecycle,
    plugin views, service manager, policy engine, and Chat Debug communicate
    through typed C4OS state and events rather than ad hoc UI coupling or
    frontend prompt parsing.
  - Thin shell and reusable harness: The persistent shell owns chat/session
    identity, Settings, plugin mounting, approvals, and shared state. Enhanced
    user experiences live in plugins so C4OS can serve as a generic AI app
    harness for coding, operations, support, research, admin, and other
    worker workflows.
  - Memory/RAM posture: Per-chat plugin instances stay lightweight. Heavy
    services start lazily, are shared at the narrowest safe scope, expose
    health/activity state, and shut down on idle, disable/uninstall, close, or
    app exit.
  - Windows compatibility: Plugin cache paths, config paths, native-module
    bindings, service lifecycle, file/trash behavior, browser/webview behavior,
    terminal behavior, and secret storage must be routed through platform
    adapters with Windows fallback or deferred-validation notes.
  - Maintainability: Implementation should preserve separate responsibilities
    for registry, manifest validation, lifecycle, migration, tool catalog,
    gateway, policy, service supervision, event fanout, shared state, and audit
    records, with documented state machines and comments for non-obvious
    platform or security behavior.

### DEC-014: 2026-07-02 POC Batch - Shell Plugin Architecture

Source: `proofs/tool-event-fanout/`, `proofs/bundled-plugin-lifecycle/`,
`proofs/plugin-migration-failure-handling/`.

  - Promote one-backend-call event fanout as feasible for visible and hidden
    compatible plugin views, with hidden views limited to state/unread updates.
  - Promote bundled/default plugin lifecycle as feasible for installed-disabled
    defaults, uninstall with data-delete prompt, and reinstall from bundled
    source.
  - Promote migration failure classification as feasible for auto-recovery,
    corrupt-config reset with notice, dependency-blocked, and disabled states.
